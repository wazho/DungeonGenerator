...
public static class Drawing
{
	...
	public static void DrawLine(Vector2 pointA, Vector2 pointB, Color color, float width, bool antiAlias, float blurry)
	{
	...
	public static void DrawCircle(Vector2 center, int radius, Color color, float width, int segmentsPerQuarter) {
		DrawCircle(center, radius, color, width, true, segmentsPerQuarter);
	}

	// Blurry default value : 1. Common use : 0.5f ~ 3.0f
	public static void DrawCircle(Vector2 center, int radius, Color color, float width, bool antiAlias, int segmentsPerQuarter, float blurry = 1.0f) {
		...
		DrawBezierLine(p1, p1_tan_b, p2, p2_tan_a, color, width, antiAlias, segmentsPerQuarter, blurry);
		DrawBezierLine(p2, p2_tan_b, p3, p3_tan_b, color, width, antiAlias, segmentsPerQuarter, blurry);
		DrawBezierLine(p3, p3_tan_a, p4, p4_tan_b, color, width, antiAlias, segmentsPerQuarter, blurry);
		DrawBezierLine(p4, p4_tan_a, p1, p1_tan_a, color, width, antiAlias, segmentsPerQuarter, blurry);
	}

	// Other than method name, DrawBezierLine is unchanged from Linusmartensson's original implementation.
	public static void DrawBezierLine(Vector2 start, Vector2 startTangent, Vector2 end, Vector2 endTangent, Color color, float width, bool antiAlias, int segments, float blurry)
	{
		Vector2 lastV = CubeBezier(start, startTangent, end, endTangent, 0);
		for (int i = 1; i < segments + 1; ++i)
		{
			Vector2 v = CubeBezier(start, startTangent, end, endTangent, i/(float)segments);
			Drawing.DrawLine(lastV, v, color, width, antiAlias, blurry);
			lastV = v;
		}
	}
